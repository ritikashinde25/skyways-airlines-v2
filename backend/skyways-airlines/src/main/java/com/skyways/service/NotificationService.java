...
public class NotificationService {
    ...
    // ─── Build HTML separately to avoid % escaping issues ─────────
    private String buildBookingConfirmationHtml(
        ...
        return "<!DOCTYPE html><html><head>"
            ...
            + "<p style=\"color:#bbdefb;margin:8px 0 0;font-size:14px;letter-spacing:1px;\">YOUR JOURNEY BEGINS HERE</p>"
            + "</div>"
            ...
            + "Below are your flight details. Please review them carefully and keep this email for your records.</p>"
            + "</div>"
            ...
            + "<p style=\"margin:0 0 8px;color:#333;font-size:13px;line-height:1.7;\">&#128272; <b>Boarding Priority:</b> Special needs passengers, families with infants, and Business Class board first.</p>"
            + "<p style=\"margin:0;color:#333;font-size:13px;line-height:1.7;\">&#128272; <b>Mobile Phones:</b> Switch to airplane mode once onboard. Wi-Fi available on select flights.</p>"
            + "</div>"
    ...
    public Notification sendCancellationNotice(String username,
        ...
        String htmlBody = "<div style=\"font-family:Arial,sans-serif;max-width:600px;margin:auto;padding:20px;\">"
            ...
            + "<p>Your booking <b>" + pnr + "</b> for flight <b>" + flightNumber + "</b> has been successfully cancelled.</p>"
            + "<p style=\"background:#fff3e0;padding:16px;border-radius:8px;border-left:4px solid #f57c00;\">"
            + "Refund will be processed within <b>5-7 business days</b> to your original payment method.</p>"
            + "<p>We hope to welcome you on board soon!</p>"
            + "<p style=\"color:#888;font-size:12px;\">For queries, call <b>1800-SKY-WAYS</b> (Toll Free)</p>"
            + "</div></div>";
        ...
        Notification notification = Notification.builder()
                ...
                .type("CANCELLATION")
                .subject(subject)
                .message("Booking " + pnr + " cancelled for flight " + flightNumber)
                .status(status)
                .sentAt(LocalDateTime.now().toString())
                .build();

...
public class NotificationService {
    ...
    public Notification sendBookingConfirmation(String username,
        ...
        Notification notification = Notification.builder()
                ...
                .subject(subject)
                .message("Booking confirmed for flight " +
                    flightNumber + " from " + origin +
                    " to " + destination)
                .status(status)
    ...
    public Notification sendCancellationNotice(String username,
        ...
        Notification notification = Notification.builder()
                ...
                .type("CANCELLATION")
                .subject(subject)
                .message("Booking cancelled for flight " +
                    flightNumber + " from " + origin +
                    " to " + destination)
                .status(status)
                .sentAt(LocalDateTime.now().toString())
    ...
    public Notification sendPasswordResetEmail(String username,
            String email, String resetLink) {
        logger.info("Sending password reset email to: {}", email);

        String subject = "Password Reset Request - SkyWays Airlines";
        String htmlBody = 
            ...
            + "SKYWAYS AIRLINES</h1></div>"
            + "<div style=\"max-width:600px;margin:30px auto;padding:0 16px;\">"
            + "<div style=\"background:#fff;border-radius:12px;padding:28px;"
            + "box-shadow:0 2px 8px rgba(0,0,0,0.08);\">"
            + "<h2 style=\"color:#0a1628;margin:0 0 16px;\">"
            + "Password Reset Request</h2>"
            + "<p style=\"color:#555;line-height:1.7;\">Dear <b>"
            + username + "</b>,</p>"
            + "<p style=\"color:#555;line-height:1.7;\">We received a request "
            + "to reset your SkyWays Airlines account password. "
            + "Click the button below to reset it.</p>"
            + "<p style=\"color:#888;font-size:13px;\">This link will expire in "
            + "<b>30 minutes</b>.</p>"
            + "<div style=\"text-align:center;margin:30px 0;\">"
            + "<a href=\"" + resetLink + "\" style=\"background:linear-gradient("
            + "135deg,#0a1628,#1a2d4f);color:#c9a84c;padding:14px 32px;"
            + "border-radius:8px;text-decoration:none;font-weight:bold;"
            + "font-size:16px;\">Reset Password</a></div>"
            + "<p style=\"color:#888;font-size:13px;\">Or copy this link:</p>"
            + "<p style=\"color:#0a1628;font-size:12px;word-break:break-all;"
            ...
            + "SkyWays Airlines | support@skywaysairlines.com | "
            + "1800-SKY-WAYS (Toll Free)</p>"
            + "</div></div></body></html>";
    ...
    private String buildBookingConfirmationHtml(String username,
        ...
        return "<!DOCTYPE html><html><body style=\"margin:0;padding:0;"
            ...
            + "<b style=\"color:#43a047;\">successfully confirmed</b>."
            + "</p></div>"
            + "<div style=\"background:#fff;border-radius:12px;padding:28px;"
            + "margin-bottom:20px;box-shadow:0 2px 8px rgba(0,0,0,0.08);\">"
            ...
            + "&#128467; <b>Check-in:</b> Online check-in opens 48 hours "
            + "before departure.<br/>"
            + "&#127968; <b>Airport Arrival:</b> Arrive at least 2 hours "
            + "before domestic flights.<br/>"
            ...
            + "&#128179; <b>Cancellation:</b> Free cancellation within 24 hours."
            + "</p></div>"
    ...
    private String buildCancellationHtml(String username,
        ...
        return "<!DOCTYPE html><html><body style=\"margin:0;padding:0;"
            ...
            + "Dear <b>" + username + "</b>,</p>"
            + "<p style=\"color:#555;line-height:1.7;margin:0;\">We are sorry "
            + "to see you go. Your booking has been "
            + "<b style=\"color:#e53935;\">successfully cancelled</b>."
            + "</p></div>"
            + "<div style=\"background:#fff;border-radius:12px;padding:28px;"
            + "margin-bottom:20px;box-shadow:0 2px 8px rgba(0,0,0,0.08);\">"
            ...
            + flightNumber + "</td></tr>"
            + "<tr><td style=\"padding:12px 16px;color:#888;font-size:13px;\">"
            + "Travel Date</td><td style=\"padding:12px 16px;"
            + "color:#333;font-weight:bold;\">" + bookingDate + "</td></tr>"
            + "<tr style=\"background:#ffebee;\"><td style=\"padding:12px 16px;"
            + "color:#888;font-size:13px;\">Route</td>"
            + "<td style=\"padding:12px 16px;color:#333;font-weight:bold;\">"
            + origin + " to " + destination + "</td></tr>"
            + "<tr><td style=\"padding:12px 16px;color:#888;font-size:13px;\">"
            + "Cancelled On</td><td style=\"padding:12px 16px;"
            + "color:#333;font-weight:bold;\">" + cancelledAt + "</td></tr>"
            + "<tr style=\"background:#ffebee;\"><td style=\"padding:12px 16px;"
            + "color:#888;font-size:13px;\">Status</td>"
            + "<td style=\"padding:12px 16px;\">"
            + "<span style=\"background:#ffcdd2;color:#e53935;padding:4px 12px;"
            + "border-radius:20px;font-size:13px;font-weight:bold;\">"
            + "CANCELLED</span></td></tr></table></div>"
            + "<div style=\"background:#fff;border-radius:12px;padding:28px;"
            + "margin-bottom:20px;box-shadow:0 2px 8px rgba(0,0,0,0.08);\">"
            + "<h3 style=\"color:#0a1628;margin:0 0 20px;"
            + "border-bottom:2px solid #c9a84c;padding-bottom:10px;\">"
            ...
            + "<p style=\"color:#2e7d32;font-weight:bold;margin:0 0 12px;\">"
            + "REFUND SUMMARY</p>"
            + "<table width=\"100%\" cellpadding=\"0\" cellspacing=\"0\""
            + " style=\"border-collapse:collapse;\">"
            + "<tr><td style=\"padding:10px;color:#555;font-size:13px;\">"
            + "Total Paid</td>"
            + "<td style=\"padding:10px;color:#333;font-weight:bold;font-size:15px;\">"
            + "INR " + (int)totalPaid + "</td></tr>"
            + "<tr style=\"background:#f1f8e9;\"><td style=\"padding:10px;"
            + "color:#555;font-size:13px;\">Refund Percentage</td>"
            + "<td style=\"padding:10px;color:#43a047;font-weight:bold;"
            + "font-size:15px;\">" + (int)refundPercentage + "%</td></tr>"
            + "<tr><td style=\"padding:10px;color:#555;font-size:13px;\">"
            + "Refund Amount</td>"
            + "<td style=\"padding:10px;color:#43a047;font-weight:bold;"
            + "font-size:18px;\">INR " + (int)refundAmount + "</td></tr>"
            + "<tr style=\"background:#f1f8e9;\"><td style=\"padding:10px;"
            + "color:#555;font-size:13px;\">Deduction</td>"
            + "<td style=\"padding:10px;color:#e53935;font-weight:bold;"
            + "font-size:15px;\">INR " + (int)(totalPaid - refundAmount)
            + "</td></tr></table></div>"
            + "<div style=\"background:#fff3e0;border-radius:8px;"
            + "padding:14px 18px;margin-bottom:16px;\">"
            + "<p style=\"color:#e65100;font-weight:bold;margin:0 0 12px;\">"
            + "REFUND TIMELINE</p>"
            + "<p style=\"color:#333;font-size:13px;line-height:1.8;margin:0;\">"
            ...
            + "&#9989; Refund confirmation email will be sent once processed."
            + "</p></div>"
            ...
            + "<b style=\"color:#43a047;\">100% refund</b><br/>"
            + "&#128197; 3-7 days before travel: <b>75% refund</b><br/>"
            + "&#128197; 1-3 days before travel: <b>50% refund</b><br/>"
            + "&#128197; Same day: "
            + "<b style=\"color:#e53935;\">No refund</b>"
            + "</p></div></div>"
